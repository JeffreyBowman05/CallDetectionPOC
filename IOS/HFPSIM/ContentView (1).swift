...
struct ContentView: View {
    ...
    private var leftPanel: some View {
        VStack(spacing: 0) {
            ...
            // QR code is shown only while advertising — the only window during
            // which a user would point their iPhone camera at the simulator.
            if peripheral.connectionState == .advertising {
                Divider()
                QRCodeView()
            }

...
// MARK: - BLE UUIDs
//
// These UUIDs must match your ESP32 provisioning firmware exactly.
...
final class BluetoothManager: NSObject, ObservableObject {
    ...
    func startProvisioning(deviceID: String) {
        ...
        // withServices is nil rather than [BLEUUID.service] because macOS
        // CBPeripheralManager does not reliably embed service UUIDs into
        // advertisement packets in a format iOS service-filtered scanning
        // recognises. Filtering is handled by name-suffix matching in
        // didDiscover. When targeting a real ESP32 this scan still finds
        // the device correctly — nil scanning works for both and is
        // required for the macOS simulator.
...
extension BluetoothManager: CBCentralManagerDelegate {
    ...
    func centralManager(
        ...
        // Name from advertisement data first — peripheral.name may be nil for
        // unseen peripherals. On macOS, this will typically be the Mac's system
        // Bluetooth name (e.g. 'Mac', 'MacBook Pro') regardless of what was
        // passed in CBAdvertisementDataLocalNameKey. That is expected and handled.
        let name = (advertisementData[CBAdvertisementDataLocalNameKey] as? String)
                   ?? peripheral.name
                   ?? ""
 
        let advertisedServices  = advertisementData[CBAdvertisementDataServiceUUIDsKey] as? [CBUUID] ?? []
        let hasProvisioningService = advertisedServices.contains(BLEUUID.service)
        let advertisesESP32Name    = name.uppercased().hasPrefix("ESP32:")
        let nameSuffixMatches      = name.uppercased().hasSuffix(id)
 
        // Primary filter: provisioning service UUID must be present.
        // Our custom UUID is specific enough to identify any device in provisioning
        // mode — no unrelated peripheral will advertise it.
        guard hasProvisioningService else { return }
 
        // Secondary filter: only enforced when the peripheral is advertising an
        // ESP32-format name (real hardware). When the name is the Mac system name
        // ('Mac', 'MacBook Pro', etc.) this check is intentionally skipped —
        // the service UUID match alone is sufficient for the simulator.
        if advertisesESP32Name && !nameSuffixMatches { return }
